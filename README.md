...
- **流程图式创作**：拖拽节点编排情节，支持主链/并行/修饰多层结构
- **多模型支持**：Ollama / LM Studio（本地）+ DeepSeek / OpenAI / 通义千问（云端 BYOK）
- **13 套快速模板**：仙侠、武侠、科幻、言情等，一键预填全部设定，直接开写
- **自动续写 & 章节管理**：自动识别章节号，逐章续写，支持中途修改设定
- **反 AI 味引擎**：内置去 AI 味提示词，输出更自然
- **多格式导出**：TXT / PDF / DOCX / EPUB
- **项目系统**：保存/加载/续写多个项目，自动存档
...
- **从模板开始**：左侧边栏点快速模板，所有节点自动填好，改项目名直接开写
- **先建流程图再生成**：拖拽节点编排故事线，效果远好于直接 prompt
- **善用续写**：写一章点一次续写，每章质量更稳定。按钮会自动变成"第二章""第三章"
- **调整推理参数**：在模型设置中调高 `temperature` 增加创意，调低增加连贯性
- **中途改设定**：写了几章后想改世界观或人物？改节点后点续写，新设定自动生效
...
[![爱发电](https://img.shields.io/badge/爱发电-支持我-946ce6)](https://ifdian.net/a/knexio)
